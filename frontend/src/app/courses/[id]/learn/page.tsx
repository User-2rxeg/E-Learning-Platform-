...
export default function CourseLearnPage() {
    ...
    useEffect(() => {
        if (
            ...
            // Auto-generate certificate when all done
            generateCertificate();
        }
    }, [progress.overallProgress, course?.certificateAvailable, certificateId, certificateGenerating]);
    ...
    }, [certificateGenerating, certificateId, courseId]);
    ...
    const fetchCourseDetails = useCallback(async () => {
        try {
            ...
            if ((user as any)?.role === 'student') {
                const enrolled = data.studentsEnrolled.includes(userId);
            ...
            } else if ((user as any)?.role === 'instructor') {
                setIsEnrolled(data.instructorId._id === userId);
            } else {
                // Default: not enrolled for unknown roles
                setIsEnrolled(false);
                router.push(`/courses/${courseId}`);
                return;
            }
    ...
    const loadProgress = useCallback(async () => {
        try {
            ...
            if (savedProgress) {
                const next: CourseProgress = {
                    ...
                    currentModule: savedProgress.currentModule ?? 0,
                    currentResource: savedProgress.currentResource ?? 0,
                    overallProgress: savedProgress.overallProgress ?? 0
                };
                setProgress(next);
                setCurrentModuleIndex(next.currentModule);
                setCurrentResourceIndex(next.currentResource);
                return;
            }
        } catch (err) {
            console.error('Error loading progress (remote):', err);
        }
        ...
        try {
            ...
            console.error('Error loading progress (local):', err);
        }
    }, [courseId]);

    const saveProgress = useCallback(
        async (newProgress: Partial<CourseProgress>) => {
            setProgress((prev) => {
                const merged: CourseProgress = { ...prev, ...newProgress };
    ...
    const loadNotes = useCallback(async () => {
        try {
            ...
            const formattedNotes: Note[] = (courseNotes ?? []).map((note: any) => ({
                id: note.id || note._id,
                moduleIndex: note.moduleIndex ?? 0,
    ...
    const markAsComplete = useCallback(async () => {
        ...
        setProgress((prev) => {
            if (prev.completedResources.includes(resourceId)) return prev;
            ...
            const overallProgress =
                totalResources > 0
                    ? Math.round((newCompleted.length / totalResources) * 100)
                    : 0;
    ...
    }, [course, courseId, currentModuleIndex, currentResourceIndex]);

    const navigateToResource = useCallback(
        async (moduleIndex: number, resourceIndex: number) => {
    ...
    }, [course, currentModuleIndex, currentResourceIndex, navigateToResource]);
    ...
    const currentResource: Resource | null = useMemo(() => {
        if (!currentModule) return null;
        return currentModule.resources[currentResourceIndex] ?? null;
    }, [currentModule, currentResourceIndex]);

    const getOverallProgress = useCallback(() => progress.overallProgress, [progress.overallProgress]);

    const isResourceCompleted = useCallback(
        (moduleIndex: number, resourceIndex: number) =>
            progress.completedResources.includes(`${moduleIndex}-${resourceIndex}`),
        [progress.completedResources]
    );

    const getResourceUrl = useCallback((resource: Resource): string => {
        if (!resource.url) return '';
        if (/^https?:\/\//i.test(resource.url)) return resource.url;

        const baseFromEnv = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3555/api';
        const base = baseFromEnv.replace(/\/+api\/?$/, '').replace(/\/+$/, '');
        const path = resource.url.replace(/^\/+/, '');
        return `${base}/${path}`;
    }, []);
        ...
        <div className="learn-container">
            ...
            <div className="learn-mainContent">
                        ...
                        <div className="learn-modulesList">
                          ...
                          {resource.filename || `${resource.resourceType} ${resourceIndex + 1}`}
                                        ...
                                        {module.quizzes && module.quizzes.length > 0 && (
                                            <Link href={`/courses/${courseId}/quiz/${module.quizzes[0]}`} className="learn-quizLink">
                                                <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path
                                                        strokeLinecap="round"
                                                        strokeLinejoin="round"
                ...
                <div className="learn-contentArea">
                                ...
                                {currentResource.resourceType === 'link' && (
                                    <div className="learn-linkContainer">
                                        <div className="learn-linkCard">
                                            ...
                                            <a
                                                href={getResourceUrl(currentResource)}
                                                target="_blank"
                    ...
                    />
                                        <button onClick={addNote} disabled={!currentNote.trim() || notesSaving}>
                                            {notesSaving ? (
                                                <>
                                ...
                                notes.map((note) => (
                                    <div key={note.id} className="learn-noteItem">
                                        <div className="learn-noteHeader">
                                            ...
                                            <span className="learn-noteTime">{new Date(note.timestamp).toLocaleDateString()}</span>
                                        </div>
